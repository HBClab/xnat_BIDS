## xnat_BIDS
[![Build Status](https://travis-ci.org/uwescience/xnat_BIDS.svg?branch=master)](https://travis-ci.org/uwescience/xnat_BIDS)
...
repository into your computer and follow the instructions at the [bottom of this page](#using-xnat_BIDS-as-a-template).
...
`xnat_BIDS/xnat_BIDS.py`. This code provided in this file is intentionally rather
...
We place the module code in a file called `xnat_BIDS.py` in directory called
`xnat_BIDS`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import xnat_BIDS as sb` in an
interactive Python session, the classes and functions defined inside of the
`xnat_BIDS.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the xnat_BIDS module, the main classes defined at the bottom of
...
Either way, you can create a `xnat_BIDS/data` folder in which you can
...
(typically, something like our `xnat_BIDS/tests/test_xnat_BIDS.py`). Within each
...
have one such test in `xnat_BIDS/tests/test_xnat_BIDS.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/xnat_BIDS`),
and type:
...
will see a message such as:
    ...
    xnat_BIDS/tests/test_xnat_BIDS.py .F...
    ...
    xnat_BIDS/tests/test_xnat_BIDS.py:49: AssertionError
...
To document `xnat_BIDS` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/xnat_BIDS/blob/master/doc/_static/RTD-advanced-conf.png)

 http://xnat_BIDS.readthedocs.org/en/latest/
...
A `xnat_BIDS/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/xnat_BIDS) for the software. This
...
`xnat_BIDS` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `xnat_BIDS`, we use the
...
[here](https://github.com/uwescience/xnat_BIDS/blob/master/scripts/Figure1.ipynb).


### Using `xnat_BIDS` as a template
...
computer under the name you will want your project to have:
	...
	git clone https://github.com/uwescience/xnat_BIDS smallish
	cd smallish
...
Next, you will want to move `xnat_BIDS/xnat_BIDS.py` to be called
`smallish/smallish.py`

	git mv xnat_BIDS smallish
	git mv smallish/xnat_BIDS.py smallish/smallish.py
	git mv smallish/tests/test_xnat_BIDS.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `xnat_BIDS` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `xnat_BIDS` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `xnat_BIDS` in them. Type
the following to see where all these files are:

	git grep xnat_BIDS

You can replace `xnat_BIDS` for `smallish` fastly with:

	git grep -l 'xnat_BIDS' | xargs sed -i 's/xnat_BIDS/smallish/g'
...
Edit all the mentions of `xnat_BIDS` in the `xnat_BIDS/__init__.py` file, and in
the `xnat_BIDS/version.py` file as well.
